...
const newPlayer = async function (query) {
...
const getPlayerProfile = async function (playerName) {
	const players = await getCollection('players')
	return players.findOne({ name: playerName })
}

const resolveGame = async function (query) {
	const valid = ajv.validate({
		type: 'object',
	...
	const gameResult = await resolveGameNvN({
...
const resolveGameNvN = async function (query) {
	const valid = ajv.validate({
		type: 'object',
...
const undoLastGame = async function () {
	const history = await getCollection('history')
	const players = await getCollection('players')
